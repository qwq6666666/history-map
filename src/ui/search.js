...
import { findAvailableLayersAt, activateFromSearch, bumpSearchToken, isSearchStale, SEARCH_ZOOM, SEARCH_RESULT_TYPES, filterAvailableByType, sortAvailableByYear } from '../features/search.js';
...
function renderAvailableLayers(available, totalChecked){
  ...
  // 篩選／排序工具列：純前端在已取得的 available 陣列上做過濾與重排，
  // 不重新呼叫 findAvailableLayersAt、不觸發任何新的網路請求。
  const toolbarEl = document.createElement('div');
  toolbarEl.className = 'avail-filter-toolbar';

  const typeRow = document.createElement('div');
  typeRow.className = 'avail-filter-row';
  const typeLabel = document.createElement('span');
  typeLabel.className = 'avail-filter-label';
  typeLabel.textContent = '類型';
  typeRow.appendChild(typeLabel);

  const sortRow = document.createElement('div');
  sortRow.className = 'avail-filter-row';
  const sortLabel = document.createElement('span');
  sortLabel.className = 'avail-filter-label';
  sortLabel.textContent = '排序';
  sortRow.appendChild(sortLabel);

  toolbarEl.appendChild(typeRow);
  toolbarEl.appendChild(sortRow);
  layerAvailPanelEl.appendChild(toolbarEl);

  const contentEl = document.createElement('div');
  layerAvailPanelEl.appendChild(contentEl);

  let currentType = 'all';
  let currentSort = null; // null：維持原始順序；'desc'／'asc'：依年代排序

  function applyFilters(){
    const filtered = filterAvailableByType(available, currentType);
    return currentSort ? sortAvailableByYear(filtered, currentSort) : filtered;
  }

  // 類型篩選按鈕（單選）："全部" + SEARCH_RESULT_TYPES
  const typeButtons = new Map();
  [['all', '全部'], ...SEARCH_RESULT_TYPES.map(t => [t, t])].forEach(([key, label])=>{
    const btn = document.createElement('button');
    btn.type = 'button';
    btn.className = 'avail-filter-btn';
    ...
    btn.addEventListener('click', ()=>{
      ...
      renderCategoryView(applyFilters());
    });
    typeButtons.set(key, btn);
    typeRow.appendChild(btn);
  });

  // 年代排序按鈕（互斥切換，可再次點擊取消回到原始順序）
  ...
  function renderCategoryView(list){
    ...
    const availableIdSet = new Set(list.map(c => c.layer.id));
    const order = [];
    const seenSrc = {};
    list.forEach(c=>{
